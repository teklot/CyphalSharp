...
{
    /// <summary>
    /// Implements the CRC-16/MCRF4XX a.k.a used for Cyphal checksum.
    /// </summary>
    public class Crc
    {
        /// <summary>
        ...
        /// <summary>
        /// Calculates the CRC-16 value for a span of bytes using table-based optimization.
        /// </summary>
        /// <param name="bytes">The span of bytes to calculate the CRC for.</param>
        /// <returns>The final CRC-16 checksum.</returns>
        public static UInt16 Calculate(ReadOnlySpan<byte> bytes)
        {
            UInt16 crc = Seed;
            for (int i = 0; i < bytes.Length; i++)
            {
                crc = (UInt16)((crc >> 8) ^ CrcTable[(crc ^ bytes[i]) & 0xff]);
            }
            return crc;
        }
    }
